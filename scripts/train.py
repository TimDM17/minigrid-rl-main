...
parser.add_argument("--target-update-interval", type=int, default=1,
                    help="target network update interval for SAC (default: 1)")
parser.add_argument("--replay-size", type=int, default=1000000,
                    help="replay buffer size for SAC (default: 1000000)")
parser.add_argument("--alpha", type=float, default=0.2,
                    help="temperature parameter alpha for SAC (default: 0.2)")
parser.add_argument("--automatic-entropy-tuning", type=bool, default=True,
                    help="automatically tune entropy coefficient for SAC (default: True)")
...
if __name__ == "__main__":
    ...
    if args.algo == "a2c":
        ...
        # Import the SACAlgo from the Added_algorithms module
    ...
    if "optimizer_state" in status:
        ...
        if args.algo == "sac":
            ...
            if "target_actor_state" in status:
                algo.target_acmodel.load_state_dict(status["target_actor_state"])
            ...
            if args.automatic_entropy_tuning and "log_alpha" in status:
                algo.log_alpha.data = status["log_alpha"]
    ...
    while num_frames < args.frames:
        ...
        if args.save_interval > 0 and update % args.save_interval == 0:
            status = {"num_frames": num_frames, "update": update,
                ...
                # Add SAC-specific states if using SAC
            if args.algo == "sac":
                status.update({
                    "critic1_state": algo.critic1.state_dict(),
                    "critic2_state": algo.critic2.state_dict(),
                    "target_critic1_state": algo.target_critic1.state_dict(),
                    "target_critic2_state": algo.target_critic2.state_dict(),
                    "target_actor_state": algo.target_acmodel.state_dict(),
                    "critic1_optimizer_state": algo.critic1_optimizer.state_dict(),
                    "critic2_optimizer_state": algo.critic2_optimizer.state_dict()
                })
                # Add alpha optimizer state if using automatic entropy tuning
                if args.automatic_entropy_tuning:
                    status["alpha_optimizer_state"] = algo.alpha_optimizer.state_dict()
                    status["log_alpha"] = algo.log_alpha.data
        ...
        # >>>>>> NEW: Save model every 50,000 frames <<<<<<
        if num_frames // 50000 > (status.get("num_frames", 0) // 50000):
            ...
            # Adding SAC-specific states if using SAC
            if args.algo == "sac":
                checkpoint.update({
                    "critic1_state": algo.critic1.state_dict(),
                    "critic2_state": algo.critic2.state_dict(),
                ...
                if args.automatic_entropy_tuning:
                    ...
                    checkpoint["log_alpha"] = algo.log_alpha.data
