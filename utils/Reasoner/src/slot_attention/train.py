...
def run(net, loader, optimizer, criterion, writer, args, train=False, epoch=0, pool=None):
    if train:
        ...
        if not args.no_cuda:
            preds_all = preds_all.to(device='cuda')
            target_all = target_all.to(device='cuda')
    ...
    for i, sample in tqdm(enumerate(loader, start=epoch * iters_per_epoch)):
        ...
        if train:
            ...
            if i % iters_per_epoch == 0:
                ...
                matched_output = utils.hungarian_matching(
                    target_set[:2], output[:2].to('cuda'), verbose=0)
                # for k in range(2):
                print(
...
def main():
    ...
    rtpt = RTPT(name_initials='HS',
                experiment_name='VFCR_slotattention_pretrain', max_iterations=args.epochs)
    rtpt.start()

    for epoch in np.arange(start_epoch, args.epochs + start_epoch):
        with mp.Pool(10) as pool:
            if not args.eval_only:
                run(net, train_loader, optimizer, criterion, writer,
                    args, train=True, epoch=epoch, pool=pool)
                ...
                scheduler.step()
            if not args.train_only:
                run(net, test_loader, None, criterion, writer,
                    args, train=False, epoch=epoch, pool=pool)
                if args.eval_only:
                    exit()
        results = {
            "name": args.name,
