...
class MiniGridEnv(gym.Env):
    ...
    def step(
        ...
        # check if the current pos is the goal position
        if crt_cell is not None and crt_cell.type == "goal":
            terminated = True
            reward = self._reward()
        # Rotate left
    ...
    def gen_obs(self):
        ...
        full_image = self.get_full_render(None,  5)
        if np.isnan(full_image).any():
            print("Warning: NaNs detected in full image, replacing them.")
        if np.isnan(image).any():
            print("Warning: NaNs detected in  image, replacing them.")

        obs = {"full_image":full_image, "image": image, "direction": self.agent_dir, "mission": self.mission, "symbolic-state":self.symbol_state,"env_symbolic_state":self.enviroment_symbolic_state}
    ...
    def get_full_render(self, highlight, tile_size):
        ...
        # Convert the image to exclude nan values
